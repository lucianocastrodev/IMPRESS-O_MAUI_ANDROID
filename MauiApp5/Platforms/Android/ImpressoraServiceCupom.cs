...
{
    public class ImpressoraServiceCupom
    {
        public void ImprimirComprovante(string nomeImpressora, ComprovanteModel pedido)
        {
            ...
            sb.Append(Encoding.UTF8.GetString(new byte[] { 0x1B, 0x45, 0x01 })); // Negrito
            sb.AppendLine(AlignRight("TOTAL FINAL:", pedido.ValorFinal.ToString("N2")));
            sb.Append(Encoding.UTF8.GetString(new byte[] { 0x1B, 0x45, 0x00 })); // Desliga negrito
            ...
            // --- Imprime QR Code antes do corte ---
            PrintQRCode(output, pedido.Numero.ToString());
